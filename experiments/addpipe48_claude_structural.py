...
from chiprl.generate_addpipe48 import (
    MAX_MASK,
    blocks_from_mask,
    write_candidate,
)
...
ADAPTER = (
    ROOT
    / "adapters"
    / "claude_addpipe48_structural.py"
)
...
def compact(row):
    return {
        "mask":
            row.get(
                "mask",
                f"0x{int(row['boundary_mask']):012x}",
            ),
...
def main():
    ...
    for query_index in range(
        ...
        print(
            ...
            best.get(
                "mask",
                f"0x{int(best['boundary_mask']):012x}",
            ),
