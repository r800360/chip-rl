...
The action space is exactly one 55-bit boundary mask. The mask represents
an ordered composition of a 56-bit adder. Bit i of the mask inserts a
...
Requirements:
- mask must be between 0x000000 and 0x7fffffffffffff
...
def main():
    ...
    if not (
        0 <= mask <= 0x7fffffffffffff
    ):
        raise SystemExit(
            "Mask outside 55-bit space"
        )
