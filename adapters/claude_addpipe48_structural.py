...
The action space is exactly one 47-bit boundary mask. The mask represents
an ordered composition of a 48-bit adder. Bit i of the mask inserts a
...
Requirements:
- mask must be between 0x000000 and 0x7fffffffffff
...
def main():
    ...
    if not (
        0 <= mask <= 0x7fffffffffff
    ):
        raise SystemExit(
            "Mask outside 47-bit space"
        )
